...
def run_rag_pipeline(samples: list[dict], collection, embedder) -> list[dict]:
    ...
    for i, s in enumerate(samples, 1):
        ...
        contexts = retrieve(q, collection, embedder)
...
def _build_ragas_llm():
    ...
    if model_env.startswith("openrouter/"):
...
def main() -> None:
    ...
    records = run_rag_pipeline(samples, collection, embedder)
